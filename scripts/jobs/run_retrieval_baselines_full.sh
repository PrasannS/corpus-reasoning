...
if [ -d "$DPR_FULL/final" ]; then
    ...
    torchrun --nproc_per_node="$NUM_GPUS" --master_port=29501 scripts/train/train_retrieval_baseline.py \
        --mode dpr \
...
if [ -d "$COLBERT_MATCHED/final" ]; then
    ...
    torchrun --nproc_per_node="$NUM_GPUS" --master_port=29502 scripts/train/train_retrieval_baseline.py \
        --mode colbert \
        --train-data "$TRAIN_DATA_MATCHED" \
        --model-name answerdotai/ModernBERT-base \
        --lr 3e-5 --epochs 1 --batch-size 256 --mini-batch-size 32 \
...
if [ -d "$COLBERT_FULL/final" ]; then
    ...
    torchrun --nproc_per_node="$NUM_GPUS" --master_port=29503 scripts/train/train_retrieval_baseline.py \
        --mode colbert \
        --train-data "$TRAIN_DATA_FULL" \
        --model-name answerdotai/ModernBERT-base \
        --lr 3e-5 --epochs 1 --batch-size 256 --mini-batch-size 32 \
